...
{
    public class AsnadParController : Controller
    {
        ...
        public AsnadParController(malisContext context, UserManager<IdentityUser> userManager, RoleManager<IdentityRole> RoleManager, SignInManager<IdentityUser> signInManager)
        {
            _context = context;
        ...
        public class empasnad
        {
            ...
            //public string Codekolv { get; set; }
            //public string Codemoinv { get; set; }
        ...
        public async Task<IActionResult> sanadpar()
        {
            ViewData["PageT"] = "خزانه";

            try
            {
                var malisContext = await _context.Pardartb.Where(p => p.Typec == 2)
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            try
            {
                ...
                var check = _context.Checkqtb.FromSql("select * from Checkqtb where nobarg not in (select isnull(Nocheckq , 0) from pardartb where typec = 2 and Nocheckq <> {0}) ", pardartb.Nocheckq).ToList();
                ViewData["listcheck"] = new SelectList(check, "Nobarg", "Nobarg", pardartb.Nocheckq);
            ...
            catch
            {
                ...
                return RedirectToAction("Index", "AsnadPar");
            }
        ...
        // more details see http://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        public async Task<IActionResult> Edit(Pardartb pardartb)
        {
            pardartb.Userid = await AuthenticateUser();
            if (pardartb.Idanbar == 0)
                pardartb.Idanbar = null;
        ...
        [HttpPost]
        public IActionResult changestatus(int mid)
        {
            try
            {
                var q = _context.Pardartb.Where(x => x.Mid == mid).FirstOrDefault();
                if (q != null)
                {
                    q.Statecheckq = 0;
                    _context.Update(q);
