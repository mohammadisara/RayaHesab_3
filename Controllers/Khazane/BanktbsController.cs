...
{
    public class BanktbsController : Controller
    {
        ...
        public BanktbsController(malisContext context, UserManager<IdentityUser> userManager, RoleManager<IdentityRole> RoleManager, SignInManager<IdentityUser> signInManager)
        {
            _context = context;
        ...
        public async Task<IActionResult> List()
        {
            try
            {
                var malisContext = _context.Banktb.Where(p => p.Typec < 3).Include(k => k.Moin.K).Include(b => b.Moin).Include(b => b.Taf);
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            var b = await _context.Banktb.Where(x => x.Mid == id).FirstOrDefaultAsync();
            if (b == null)
                return NotFound();
            ...
            try
            {
                var malisContext = await _context.Masterchack.Where(p => p.Bankid == id)
                    .Include(x => x.Bank).ToListAsync();
                ViewBag.bankname = b.Title;
                return View(malisContext);
            }
            catch (Exception err)
            {
                ViewData["err"] = err.Message + (err.InnerException != null ? " ** " + err.InnerException.Message : "");
        ...
        [HttpPost]
        public async Task<IActionResult> savecheck(Masterchack masterchack)
        {
