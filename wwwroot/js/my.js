var lmsg = "در حال بارگذاری لطفا منتظر بمانید";
...
function loadcoding() {
    $.ajax({
        ...
        dataType: "json",
        success: function (data) {
            $('#txtmoin').autocomplete({
                appendTo: "#MyModal",
                source: data,
                select: function (event, ui) {
                    $("#namemoin").html(ui.item["tafname"] + ' - ' + ui.item["moinname"]
                        + ' - ' + ui.item["kolname"]);
                    $("#hfmoinid").val(ui.item["id"].split("*")[0]);
                    $("#hftafid").val(ui.item["id"].split("*")[1]);
                }
            });
        }
    });
    $.ajax({
        ...
        success: function (data) {
            $('#txtperson').autocomplete({
                ...
                source: data,
                select: function (event, ui) {
                    $("#txtperson").html(ui.item["namekamel"]);
                    $("#hfpersonid" ).val(ui.item["id"]);
                }
            });
        }
    });
...
function loadcodingBes() {
    $.ajax({
        ...
        dataType: "json",
        success: function (data) {
            $('#txtmoinS').autocomplete({
                appendTo: "#MyModal",
                source: data,
                select: function (event, ui) {
                    $("#namemoinS").html(ui.item["tafname"] + ' - ' + ui.item["moinname"]
                        + ' - ' + ui.item["kolname"]);
                    $("#hfmoinidS").val(ui.item["id"].split("*")[0]);
                    $("#hftafidS").val(ui.item["id"].split("*")[1]);
                }
            });
        }
    });
    $.ajax({
        ...
        dataType: "json",
        success: function (data) {
            $('#txtpersonS').autocomplete({
                appendTo: "#MyModal",
                source: data,
                select: function (event, ui) {
                    $("#txtpersonS").html(ui.item["namekamel"]);
                    $("#hfpersonidS").val(ui.item["id"]);
                }
            });
        }
    });
